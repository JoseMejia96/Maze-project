...
function jsonMaze(data){
	data.forEach(x=>maze.push(x.linea));
}
...
$(document).keyup(function(e){
    if((e.which == 83) && canMove(player.x, player.y-1))//Up arrow
        player.y--;
    else if((e.which == 87) && canMove(player.x, player.y+1)) // down arrow
        player.y++;
    else if((e.which == 65) && canMove(player.x-1, player.y))
        player.x--;
    else if((e.which == 68) && canMove(player.x+1, player.y))
        player.x++;
    draw();
    e.preventDefault();
});
